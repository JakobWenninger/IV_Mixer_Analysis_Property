...
"""
import sys
sys.path.append('/Users/wenninger/OneDrive - Nexus365/PhD_Oxford/Python_Scripts/QMix/QMix-master')
...
class RawData(object):
    ...
    def _recover_zemb(self):
        ...
        self.params_alpha = [self.dciv, exp_voltage, exp_current, vph, self.kwargs]
        self.find_alphas_return = _find_alpha(self.dciv, exp_voltage, exp_current, vph, **self.kwargs)
        ...
        zt_real = np.linspace(remb_range[0], remb_range[1], 5001)#Edited Jakob 2019/12/16
        zt_imag = np.linspace(xemb_range[0], xemb_range[1], 10001)#Edited Jakob 2019/12/16
        ...
        self.alphas = alpha# Jakob 2019/12/17
    ...
    def plot_error_surface(self, fig_name=None, ax=None):  # pragma: no cover
        ...
        zt_real = np.linspace(remb_range[0], remb_range[1], 101) * self.rn #Edited Jakob 2019/12/16
        zt_imag = np.linspace(xemb_range[0], xemb_range[1], 201) * self.rn #Edited Jakob 2019/12/16
...
def _find_alpha(dciv, vdc_exp, idc_exp, vph, alpha_max=1.5, num_b=20, **kw):
    ...
    alpha_step = alpha_max / 3. #4 initially Jakob 2019/12/17
    for it in range(1000): #Jakob Changed to 40, has no effect; 2019/12/17
