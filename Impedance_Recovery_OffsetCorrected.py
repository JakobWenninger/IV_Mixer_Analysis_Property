...
def alphaDeterminationAtEachBias(Unpumped,Pumped,fLO,n):
    ...
    def pumpedFunction(alpha,bias, unpumped,pumped, vPh,n):
        ...
        ''' 
        n= np.arange(-n,n+1)
        bessel = jv(n,alpha)**2 # only positive values due to square
        unpumpedOffseted = []
        for nx in n:
            unpumpedOffseted.append(unpumped[1,(np.abs(unpumped[0]-(bias+nx*vPh))).argmin()])
        return np.abs(np.nansum(bessel*unpumpedOffseted)-pumped[1,(np.abs(pumped[0]-(bias))).argmin()])
        ...
        #return np.nansum(np.abs(valueMinimised))
        #return np.abs(valueMinimised)
    pumped = Pumped.offsetCorrectedBinedIVData   
    vPh = const.h*fLO/const.e *1e3 # mV
    
    #compute the whole voltage range, relevant for the Kramers Kronig Transformation
    voltageLimit = np.abs([Unpumped.offsetCorrectedBinedIVData[0,-1]+n*vPh,Unpumped.offsetCorrectedBinedIVData[0,0]-n*vPh]).max()
...
Unpumped = IV_Response('DummyData/John/Unpumped.csv',columnOffset=0,headerLines=1,currentFactorToMicroampere=1000,rNThresholds = [3.2,6],numberOfBins=2501,vmin=-6,vmax=6)
Pumped = IV_Response('DummyData/John/Pumped.csv',columnOffset=0,headerLines=1,currentFactorToMicroampere=1000,rNThresholds = [3.7,6],numberOfBins=2501,vmin=-6,vmax=6)
...
unpumped = Unpumped.offsetCorrectedBinedIVData
pumped = Pumped.offsetCorrectedBinedIVData


#alphasWithoutCorrection = alphaDeterminationAtEachBiasWithoutNormalResistanceCorrection(unpumped,pumped,fLO,n)
alphas = alphaDeterminationAtEachBias(Unpumped,Pumped,fLO,n)
...
iACRe=ifreqRe(alphas,Unpumped,vPh,n)
...
def iKKcalc(ivData):
    ...
    array of size of V0
        Kramers Kronig Transformed Currents
    '''
    # TODO remove hard coded normal resistance
    return np.array([ivData[0],-hilbert(ivData[1]-ivData[0]*1e3/13.676).imag]) #TODO minus sign? subtraction of V/Rn?
    #return np.array([ivData[0],-hilbert(ivData[1]-ivData[0]*1e3/13.8).imag]) #TODO minus sign? subtraction of V/Rn?
    #return np.array([ivData[0],-hilbert(ivData[1]).imag]) #TODO minus sign? subtraction of V/Rn?

def ifreqImWithoutCorrections(alphas, Unpumped,vPh,n):
    '''The real AC current caused by the first photon step (n=1).
    This is done on behalf of the unpumped DC IV curve and the pumping level at each bias voltage alpha
    ...
    array of shape V0
        The real part of the AC current
    '''
    iKK = iKKcalc(Unpumped.offsetCorrectedBinedIVData)
    #plt.plot(iKK[0],iKK[1])
    #plt.plot(Unpumped.offsetCorrectedBinedIVData[0],Unpumped.offsetCorrectedBinedIVData[1])
    #plt.show()
...
iACIm=ifreqIm(alphas,Unpumped,vPh,n)
#iACImWOCorrection=ifreqImWithoutCorrections(alphasWithoutCorrection,Unpumped,vPh,n)
...
def ySIS(iSISRe,iSISIm):
    ...
    y = np.divide((iSISRe[1] + 1j* iSISIm[1,indexes]),iSISRe[0]*1e3)
    y = np.vstack([iSISRe[0],y])
    return y[:,np.logical_not(np.isinf(y[1]))]

ySISarray = ySIS(iACRe,iACIm)

def mask_steps(Unpumped,vPh, nSteps=1):
    ...
    maskWidth = np.average(Unpumped.gaussianBinSlopeFit[:,1]*8) #use 4 sigma per side to be sure
    n = np.arange(-nSteps,nSteps)
    lowerBoundaries = n*vPh+maskWidth +Unpumped.gapVoltage
    upperBoundaries = np.negative(n)*vPh-maskWidth + Unpumped.gapVoltage
    upperBoundaries = upperBoundaries[::-1] # sort it after increasing voltages
    ret = []
    for i in np.arange(2*nSteps):#range does not work in this cased
        ret.append(Unpumped.offsetCorrectedBinedIVData[0,np.logical_and(np.abs(Unpumped.offsetCorrectedBinedIVData[0])>lowerBoundaries[i],
                                                                        np.abs(Unpumped.offsetCorrectedBinedIVData[0])<upperBoundaries[i])])
    return np.hstack(ret)
        
def plot_mask_steps(Unpumped,Pumped,vPh, nSteps=1):
    ...
    nSteps: int
        The number of steps which should be masked out.
    '''
    plot(Unpumped.offsetCorrectedBinedIVData)
    plot(Pumped.offsetCorrectedBinedIVData)
    ymin = Unpumped.offsetCorrectedBinedIVData[1].min()
    ymax = Unpumped.offsetCorrectedBinedIVData[1].max()
    vspan = mask_steps(Unpumped,vPh, nSteps)
    plt.vlines(vspan,ymin,ymax,alpha=.2)
...
def findYemb(ySIS):
    ...
    def errorFunction(yLO,ySIS,dummy):
        ...
        errorfunc = []
        errorfunc.append( np.sum(np.square(ySIS[0]*1e3))) # single value
        errorfunc.append( np.square(np.abs(iLO))*np.sum(np.reciprocal(ySS)))
        errorfunc.append( -2*np.abs(iLO)*np.sum(np.divide(ySIS[0],np.sqrt(ySS))))
        #print(errorfunc)
        return np.sum(errorfunc).real # the imaginary part is always 0
        
    guess = [.05,-.05,1] # Does not take complex value as input
    return fmin(errorFunction,guess,args=(ySIS,1))

#yEmb = findYemb(ySISarray)
#yEmb1 = findYemb(ySISarray[:,np.where(np.logical_and(ySISarray[0]>1.,ySISarray[0]<1.6))])
#yEmb2 = findYemb(ySISarray[:,np.where(np.logical_and(ySISarray[0]>1.9,ySISarray[0]<2.6))])

def findYemb_From_Steps(ySIS,Unpumped,vPh,nSteps=1):
    ...
    list:
        fmin result including the embedding admittance.    
    '''
    vrangeEvaluated = mask_steps(Unpumped,vPh, nSteps)
    return findYemb(ySIS[:,np.isin(ySIS[0],vrangeEvaluated)])
